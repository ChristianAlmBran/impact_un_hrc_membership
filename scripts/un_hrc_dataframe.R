...
gdp_per_capita_ppp <- read.csv("gdp_per_capita_ppp.csv")
...
# Adding the mass mobilization column
## Selecting only mass mobilization from vdem
mass_mob <- vdem %>%
  select(country_name, year, historical, v2cagenmob) %>%
  filter(historical == 0)

## Selecting columns
mass_mob <- mass_mob %>%
  select(country_name, year, v2cagenmob)

## Renaming columns 
mass_mob <- mass_mob %>%
  rename(countries = country_name, mass_mob = v2cagenmob)

## Merging the mass_mob df and the main_df
main_df <- main_df %>%
  left_join(mass_mob, by = c("countries", "year"))
...
model <- lm(civil_liberties_index ~ membership + world_civil_liberties_index +
              ...
              mass_mob + democracy_index, data = main_df)
summary(model)

check_collinearity(main_df)
